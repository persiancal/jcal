...
void set_cal_matrix(struct cal_layout *l, struct jtm *ct,
  ...
  m = (jalali_is_jleap(mb.tm_year) && mb.tm_mon == 11)
          ? 30
  ...
  for (i = 0, c = 0; i <= mat->height && c < m; i++) {
    for (; j <= 6 && c < m; j++) {
      if ((mb.tm_year == lt.tm_year) && (mb.tm_mon == lt.tm_mon) &&
        ...
        mat->m[i][j + _prefix] =
            (l->julian) ? 1000 + mb.tm_yday + c + 1 : 1000 + mb.tm_mday + c;
      } else {
        mat->m[i][j + _prefix] =
            (l->julian) ? mb.tm_yday + c + 1 : mb.tm_mday + c;
      }
      c++;
    }
    j = 0;
    ...
    for (j = 0; j < mat->width; j++) {
      ...
      if (m == 1) {
        ...
        if (mat->m[i][j] == 0)
          ...
          if (l->farsi) {
            jalali_to_farsi(buf, 100, 2 + l->julian, " ", mat->m[i][j] - 1000);
          } else {
            snprintf(buf, 100, "%*d", 2 + l->julian, mat->m[i][j] - 1000);
          }
          ...
          if (l->farsi) {
            jalali_to_farsi(buf, 100, 2 + l->julian, " ", mat->m[i][j]);
          } else {
...
void show_cal(struct cal_layout *l, struct cal_matrix *m, struct jtm **_j) {
  ...
  if (l->farsi)
    ptr_d = (l->julian) ? (char **)fa_jalali_days_3 : (char **)fa_jalali_days_2;
  else if (l->english)
    ptr_d = (l->julian) ? (char **)jalali_days_3 : (char **)jalali_days_2;
  else
    ptr_d = (l->julian) ? (char **)jalali_days_3_fa : (char **)jalali_days_2_fa;

  for (i = 0; i < m->n; i++) {
    if (l->farsi) {
      jalali_to_farsi(buf, 100, 0, " ",
                      _j[i]->tm_year +
                          ((l->pahlavi) ? PAHLAVI_ISLAMIC_DIFF : 0));
    } else {
      snprintf(buf, 100, "%d",
               _j[i]->tm_year + ((l->pahlavi) ? PAHLAVI_ISLAMIC_DIFF : 0));
    }
    snprintf(cal_y[i], 100, "%s%s", buf,
             (l->pahlavi) ? ((l->farsi) ? " په" : "(pa)") : "");
    buf[0] = 0;
  }

  for (i = 0; i < m->n; i++) {
    snprintf(cal_t[i], MAX_BUF_SIZE, "%s %s",
             (l->farsi) ? fa_jalali_months[_j[i]->tm_mon]
...
void show_3(struct cal_layout *l, struct jtm *j) {
  ...
  diff_n =
      (jalali_is_jleap(_j[1]->tm_year) && _j[1]->tm_mon == 11)
...
void show_year(struct cal_layout *l, struct jtm *j) {
  ...
  int cal_width =
      (((l->julian) ? (3 * 7 + 6) : (2 * 7 + 6)) * 3) + (2 * l->margin);
  ...
  if (l->farsi) {
    jalali_to_farsi(buf, 100, 0, " ",
                    j->tm_year + ((l->pahlavi) ? PAHLAVI_ISLAMIC_DIFF : 0));
  } else {
    snprintf(buf, 100, "%d",
             j->tm_year + ((l->pahlavi) ? PAHLAVI_ISLAMIC_DIFF : 0));
  }
